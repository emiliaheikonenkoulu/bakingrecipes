...
public class BakingRecipesApplication {
	...
	@Bean
	public CommandLineRunner RecipeDemo(RecipeRepository recipeRepository, CategoryRepository categoryRepository, SpecialDietRepository specialDietRepository, UserRepository userRepository) { 
		return (args) -> {
			...
			recipeRepository.save(new Recipe("Trio-kakku", "Kesäinen trio suklaa kakku maistuu varmasti kaikille!", "Helppo",
			"Oreo-keksejä, voita, kermaa, tuorejuustoa, tumma-, valko- ja maitosuklaa, liivatelehti, sokeri ja mansikoita.",
			categoryRepository.findByName("Kakut").get(0), specialDietRepository.findByName("Pähkinätön").get(0)));
			recipeRepository.save(new Recipe("Prinsessapullat", "Viehättävä, värikäs sekä herkullinen yhdistelmä.", "Vaativampi",
